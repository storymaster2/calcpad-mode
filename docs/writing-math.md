...
It is also very useful for writing Html code. **CalcpadCE** syntax can be also used with Notepad++. It is predefined as an XML file that can be inserted in Notepad++. You can do this by selecting the "**Language**" menu, then click "**Define your language**" and then, "**Import…**". Find the **CalcpadCE** folder inside your **Program Files** directory or wherever CalcpadCE is installed and select the file named [Calcpad-syntax-for-Notepad++.xml](https://calcpad.eu/download/Notepadpp.zip).
