...
-   **Bridge duplication.** [message-bridge.ts](../frontend/calcpad-web/src/services/message-bridge.ts) and [neutralino-bridge.ts](../frontend/calcpad-web/src/services/neutralino-bridge.ts) contain two parallel `MessageBridge` classes with ~40 near-identical handlers (`handleGetSettings`, `handleUpdateSettings`, `handleGetVariables`, `handleGeneratePdf`, `handleSaveSourceHtml`, …). Only the storage/IPC backend differs.
...
-   **Scattered PDF-settings defaults.** Two identical 12-field default objects at [message-bridge.ts:277-296](../frontend/calcpad-web/src/services/message-bridge.ts#L277-L296) and [neutralino-bridge.ts:768-787](../frontend/calcpad-web/src/services/neutralino-bridge.ts#L768-L787).
...
-   **Image-insert helpers duplicated.** [image-insert.ts:32-54](../frontend/calcpad-web/src/services/image-insert.ts#L32-L54) vs [imageInserter.ts](../frontend/vscode-calcpad/src/imageInserter.ts) vs inline MIME map in the Neutralino bridge.
-   **Regex state hazard.** Global regex reused without resetting `lastIndex` at [neutralino-bridge.ts:83](../frontend/calcpad-web/src/services/neutralino-bridge.ts#L83).
...
2. `MessageBridge` (web) and `NeutralinoMessageBridge` (desktop) become ~50-line adapters implementing those interfaces.
...
3. Reset regex `lastIndex` before each use in `neutralino-bridge.ts` (or construct the regex per-call — it's not hot).
