...
# Build script for Calcpad.Server Linux
# Builds the .NET application with integrated PDF generation (PuppeteerSharp + PDFsharp)
...
echo -e "${BLUE}Building Calcpad.Server Linux...${NC}"
...
# Build for Linux x64 
echo -e "${YELLOW}Building Linux x64 application...${NC}"
...
if [ -f "$PUBLISH_PATH/Calcpad.Server" ]; then
    ...
    echo -e "${GREEN}  Linux: Copy files to a Linux machine and run${NC}"
    echo -e "${GREEN}  Docker: docker build -f Dockerfile -t calcpad-server .${NC}"
    ...
    echo -e "${YELLOW}  • PDF generation with PuppeteerSharp + PDFsharp${NC}"
    echo -e "${YELLOW}  • JWT authentication with SQLite user database${NC}"
    echo -e "${YELLOW}  • Automatic browser detection (Chromium)${NC}"
    echo -e "${YELLOW}  • File logging to working directory${NC}"
